...
**History & skip system.** Every downloaded file is recorded in `history_files/<folder_id>.txt`. Skip decisions are based on that history file, **not on what's on disk** — deleting downloaded files won't cause them to be re-fetched. This is deliberate: it lets you re-scan an account later and only pick up files added since your last run.
...
docker run -p 8000:8000 -v $(pwd)/data:/data --env-file .env 4shared-downloader
```

`DOWNLOADER_DATA_DIR` (defaults to `/data` in the container) controls where `myaccounts.txt`, `downloads/`, `history_files/`, and `download_log.txt` live — point it at a mounted volume so data survives restarts. If a bind-mounted host directory isn't writable inside the container, `chown` it to UID 1000 or run with `--user $(id -u):$(id -g)`.
