...
for browsing history/results (history_files/, downloads/, download_log.txt)
...
from app.core.paths import resolve_dest_root
...
class Job:
    ...
    # Per-run filter/limit/destination options, set at trigger time — not
    # persisted per-account, just carried on the Job for display/debugging.
...
class JobManager:
    ...
    async def start(self, folder_ids: list[str], name_filter: str | None = None,
                     file_type: FileType = "all", limit: int | None = None,
                     dest_dir: str | None = None) -> Job:
        ...
        # Resolved before the Job exists so a bad path fails fast (HTTP 400)
        # rather than deep inside the async job. Run in a thread since the
        # whole point of this feature is a *mounted* path — a stalled
        # network mount must not block the event loop (and therefore the
        # SSE stream for every connected client).
        dest_root = await asyncio.to_thread(resolve_dest_root, dest_dir, config.DOWNLOAD_DIR)

        job = Job(id=str(uuid.uuid4()), folder_ids=folder_ids, name_filter=name_filter,
                  file_type=file_type, limit=limit, dest_dir=dest_dir)
        self._jobs[job.id] = job
        self._active_job_id = job.id

        loop = asyncio.get_running_loop()
        handler = _QueueLogHandler(job, loop)
        job.task = asyncio.create_task(self._run(job, handler, dest_root))
        return job

    async def _run(self, job: Job, handler: logging.Handler, dest_root) -> None:
        root_log = logging.getLogger()
        root_log.addHandler(handler)
        job.status = JobStatus.RUNNING
        job.started_at = datetime.now(timezone.utc)
        log.info(f"Destination root for this run: {dest_root}")
        try:
            options = CrawlOptions(name_filter=job.name_filter, file_type=job.file_type, limit=job.limit)
            ...
            for folder_id in job.folder_ids:
                ...
                # folder_id_from_url() (app/core/urls.py) can never produce
                # ".." or a path separator, so this join stays inside
                # dest_root even though dest_root itself is user-supplied.
                dest_dir = dest_root / folder_id
                ...
                try:
                    await run_account(account.url, dest_dir, folder_id, config.HISTORY_DIR,
                                       downloaded, config.METADATA_FILE, options)
