...
from app.core.history import load_history, metadata_history_path
...
async def main() -> None:
    ...
    for account in accounts_to_run:
        ...
        log.info(f"  Metadata file   : {metadata_history_path(config.HISTORY_DIR, account.folder_id)}")
        log.info(f"  Already seen    : {len(downloaded)} file(s) — strictly skipped")
        log.info(f"{'='*60}")

        try:
            await run_account(account.url, dest_dir, account.folder_id, config.HISTORY_DIR, downloaded)
